...
class Picoscope5000a():
    ...
    def _store_data_to_disk(self,):
        for ch in self.channels.values():
            # Save driver buffer to program buffer
            ch.buffer_total[self.nextSample:self.destEnd] = ch.buffer_small[self.startIndex:self.sourceEnd]
            # Convert and save latest data to storage
            np.savetxt(ch.saving_file, 
                        self.convert_ADC_numbers(ch.buffer_small[self.startIndex:self.sourceEnd],ch.vrange, ch.irange),
                        delimiter = '\t')


    def _signal_processing(self, samples_to_process:int):
        # Save driver buffer to program buffer
        for ch in self.channels.values():
            ch.buffer_total[self.nextSample:self.destEnd] = ch.buffer_small[self.startIndex:self.sourceEnd]
        self.tail = self.destEnd # !!! This does not take into account the circularity!!! WRITE A BUFFER OBJECT
        # Process data to compute transfer function and downsample
        if (self.tail - self.head) > samples_to_process:
            signals = []  
            # Pop data from program buffer
            for ch in self.channels.values():
                signals.append(self.convert_ADC_numbers(ch.buffer_total[self.head:self.tail],ch.vrange, ch.irange))
            self.head = self.tail
        # Perfom STFTEIS on the data
        # Save high frequency impedance
    ...
    def streaming_callback(self, 
        ...
        self.sourceEnd = startIndex + noOfSamples
        self.destEnd = self.nextSample + noOfSamples
        self.nextSample += noOfSamples
        if self.method == 'store_to_disk':
            self._store_data_to_disk()
        elif self.method == 'filter_and_downsample':
            ...
        elif self.method == 'compute_tf_and_downsample':
            self._data_process_fft()
        for ch in self.channels.values():
            # ch.buffer_total[self.nextSample:destEnd] = ch.buffer_small[startIndex:sourceEnd]
            ch.signal.append(ch.buffer_small[startIndex:self.sourceEnd])
        self._data_process()
